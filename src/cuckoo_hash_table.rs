...
#[derive(Debug)]
pub struct CuckooHashMap<K, V: Hash, const N: usize> {
    cuckoo_table: RawCuckooTable<K, V, N>,
}
impl<K, V: Hash, const N: usize> CuckooHashMap<K, V, N> {
    ...
    /// ```
    pub fn new() -> CuckooHashMap<K, V, N> {
        todo!()
    }
    ...
    /// ```
    pub fn with_capacity(capacity: usize) -> CuckooHashMap<K, V, N> {
        todo!()
    }
    ...
    /// ```
    #[inline]
    pub fn hashers(&self) -> [RandomState; N] {
        todo!()
    }
...
#[derive(Debug)]
struct RawCuckooTable<K, V, const N: usize> {
    /// We use 4 hash tables of equal size
    hash_tables: Option<[Box<[CuckooTableSlot<K, V>]>; N]>,

    /// hash functions for each of our 4 hash tables
    hash_functions: Option<[RandomState; N]>,
...
impl<K, V, const N: usize> RawCuckooTable<K, V, N> {
    /// create a new empty cuckoo hash table
    pub fn new() -> RawCuckooTable<K, V, N> {
        RawCuckooTable {
            hash_tables: None,
            overflow: None,
            hash_functions: None,
...
impl<K, V> CuckooTableSlot<K, V> {
    ...
    /// The return type indicates if we succeeded without eviction, or
    /// if we succeeded via eviction
