...
//! assumed that the underlying array is static and that we plan to make
...
//! run-times by first preprocessing it. This module pre-processes
//! the array using the Fisher-Heun Structure which uses the method of four russians.
//! Specifically, it pre-processes the array by first partitioning the input
...
//! a new auxillary summary array S. It then builds a sparse table of RMQ
//! answer for S. Finally, on each of the partitions, it pre-computes
...
/// We use this to store and retrieve entries in our
...
impl<T: Ord + Copy> FischerHeunRMQ<T> {
    ...
    ///
    /// ```
    /// static_array.len() = 7 and block_size=3
    /// i = 0, 1, 2, 3, 4, 5, 6
    ...
    /// This procedure uses the dp algorithm `compute_rmq_all_ranges` to pre-compute
